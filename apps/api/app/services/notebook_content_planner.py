...
def _section_is_llm_eligible(section_id: str, llm_client=None) -> bool:
    ...
    if section_id in {"segment_and_region", "country_market", "order_structure"}:
        # This section already has a stronger deterministic fallback, and the
        # local proxy path can be less stable on these wider payloads.
        return bool(
            llm_client is not None
            and getattr(llm_client, "source", "") != "cliproxy"
        )
